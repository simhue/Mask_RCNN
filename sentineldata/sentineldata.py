...
def download_sentinel_products_for_ROI(geojson_file):
    ...
    while True:
        ...
        if cloudcoverpercentage >= max_cloudcoverpercentage or len(products) > 0:
...
def getvci(ndvi_band):
    return (ndvi_band - np.nanmin(ndvi_band)) / (np.nanmax(ndvi_band) - np.nanmin(ndvi_band))
